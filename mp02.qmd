...
![](/docs/MP02/mp02_files/figure-html/unnamed-chunk-3-1.png)
...
TITLE_BASICS <- TITLE_BASICS %>%
  mutate(
    startYear = as.numeric(startYear),  # NA values introduced due to non-numeric data
    endYear = as.numeric(endYear),      # Same here for endYear
    isAdult = as.logical(isAdult)       # Assuming 1 = TRUE, 0 = FALSE
  )
...
1.  What was the genre with the most “successes” in each decade?
    ...
    ![](Success%20by%20Genre%20and%20Decade)
...
![](Top%2010%20Actors%20and%20Directors%20in%20Documentary%20Films)
